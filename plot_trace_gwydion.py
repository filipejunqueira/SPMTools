...
file_path = "/media/captainbroccoli/DATA/thesis_figs/traces/598_height_difference.txt"
...
colours_palette = ["#D13A28","#00C9A7", "#0057D9", "#5B6C5D","#7b40c9", "#FFC145",]
#plt.title(plot_title)

index = 0
minimization_cutoff = 20

while index < number_traces:

    x = np.array(trace[:, 2 * index])*1000_000_000_000
    y = np.array(trace[:, 2 * index + 1])*1000_000_000_000
    y = y - np.nanmin(y[:minimization_cutoff])
    sns.scatterplot(ax=axis, x=x, y=y, alpha=1, edgecolor=colours_palette[index%5], facecolor="None", s=marker_size)
