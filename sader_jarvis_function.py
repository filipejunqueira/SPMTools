...
def sjarvis_benchmark(zmin=0.23E-9,zmax=5.000E-9,points =5000, sigma = 0.235E-9,E = 0.371E-18,f0 = 32768,k = 1800,A =11.8E-12, simple=False, plot=False):
    ...
    if simple==False:
        ...
        df_on = pre_factor*(((sigma / z)**(7)) * (Fy(7, 0.5, 1, (-2 * A / z)) - Fy(7, 1.5, 2, (-2 * A / z)))) - pre_factor*(((sigma / z)**(13))*(Fy(13, 0.5, 1, (-2 * A / z)) - Fy(13,1.5,2,(-2 * A / z) )))
...
#min_1=sjarvis_benchmark(zmin=-1.0E-9,zmax=-0.01E-9,A=10E-12,simple=False,plot=True)
...
#    force, delta_f, omega, dz_omega, z_force = sjarvis_deconvolution(df_ON=cu_ON,df_OFF=cd_OFF,A=A,f0=f0,k=k)
